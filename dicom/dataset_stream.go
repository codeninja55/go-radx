...
// dataset proper. For Deflated Explicit VR LE the element stream is deflated. An unrecognised
// or empty transfer syntax is rejected before any bytes are written (Codex DCM-002).
...
// read from the stream is validated against a bounded reader before allocation (PRD §9.3). A
// recognised encapsulated syntax retains the (7FE0,0010) fragment stream on the dataset; an
// unrecognised syntax is rejected.
