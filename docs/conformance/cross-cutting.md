...
and community conduct follows [`CODE_OF_CONDUCT.md`](../../CODE_OF_CONDUCT.md). The versioning and deprecation policy
that turns the experimental posture into a predictable contract is authored in
[`docs/development/versioning.md`](../development/versioning.md): semantic versioning over git tags, the `0.x`
breaking-changes-between-minors latitude, the per-package `experimental`/`stabilising` tiers (every package is
`experimental` today), the deprecation policy, and the `/vN` module-path rule for a future major version.

### Release and supply-chain posture

go-radx the library is consumed as a Go module and is verified by the module proxy
(`proxy.golang.org` and `sum.golang.org`); it has no binary release. The one binary artifact is the `radx` CLI,
released from a separate Go module at `cmd/radx`. The tag-triggered release pipeline at
[`.github/workflows/release.yml`](../../.github/workflows/release.yml) builds the CLI with GoReleaser
([`cmd/radx/.goreleaser.yaml`](../../cmd/radx/.goreleaser.yaml)) and produces, for every `v*` tag: cross-platform
archives (linux, macOS, Windows on amd64 and arm64) with the version, commit, and date stamped into `radx --version`
via linker flags; a `checksums.txt` of every archive; a `cosign` keyless signature over that checksums file (Sigstore
over the GitHub Actions OIDC token, no stored key); a CycloneDX and an SPDX SBOM per archive generated by `syft`; and a
SLSA build-provenance attestation per artifact via `actions/attest-build-provenance`. A separate validation job runs
`goreleaser check` and a no-publish `goreleaser release --snapshot --clean` on every pull request, so the release
configuration is exercised before any tag is pushed. These claims describe what the workflow does; they apply to the
`radx` CLI artifacts only, not to the library module.
