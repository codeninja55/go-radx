...
| Retrieve bulkdata | §10.4 | MET | library: `retrieve.go:336` via `BulkDataRetriever` (:94) plus `resolveBulkDataLocator` (:391); daemon: `role_dicomweb.go:337` `RetrieveBulkData` | - | A locator-suffixed URI (the form metadata emits) returns exactly the referenced attribute, top-level and nested sequence paths alike; the bare `.../bulkdata` sub-resource returns every binary value; an unresolvable locator answers 404, never wrong bytes |
