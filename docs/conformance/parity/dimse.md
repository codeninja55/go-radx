...
| N-EVENT-REPORT receive (role-inverted acceptor) | storage commitment examples | PARTIAL | `NEventReportHandler` `dimse/nhandler.go` routed by `dimse/dispatch.go` (CommandNEventReportRQ arm) via `serveNEventReportMessage` `dimse/ndispatch.go`; `CommitmentReceiver.ServeConn`/`serveReport` `dimse/stgcommit.go` | M | General N-EVENT-REPORT SCP via `Server`+`NEventReportHandler` (any negotiated SOP class) plus the Storage-Commitment-typed `CommitmentReceiver`; handler returns Status only - no Event Reply data set in the RSP |
