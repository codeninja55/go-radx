...
type Endpoint struct {
	...
	PayloadMimeTypeElement []*fhir.PrimitiveElement `json:"-"`
...
func marshalEndpointSiblings(v *Endpoint, encoded []byte) ([]byte, error) {
	...
	if raw, err := fhir.MarshalPrimitiveExtensions(len(v.PayloadMimeType), v.PayloadMimeTypeElement); err != nil {
		return nil, err
	} else {
		siblings = append(siblings, fhir.RawSibling{Key: "_payloadMimeType", Value: raw})
	}
...
func (v *Endpoint) UnmarshalJSON(data []byte) error {
	...
	if raw, ok := fhir.TakeRawField(obj, "_payloadMimeType"); ok {
		elements, err := fhir.UnmarshalPrimitiveExtensions(raw)
		if err != nil {
			return err
		}
		v.PayloadMimeTypeElement = elements
	}
