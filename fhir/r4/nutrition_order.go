...
type NutritionOrderEnteralFormula struct {
	...
	AdditiveProductNameElement       *fhir.PrimitiveElement                       `json:"-"`
...
func marshalNutritionOrderEnteralFormulaSiblings(v *NutritionOrderEnteralFormula, encoded []byte) ([]byte, error) {
	...
	if raw, err := fhir.MarshalPrimitiveExtension(v.AdditiveProductNameElement); err != nil {
		return nil, err
	} else {
		siblings = append(siblings, fhir.RawSibling{Key: "_additiveProductName", Value: raw})
	}
...
func (v *NutritionOrderEnteralFormula) UnmarshalJSON(data []byte) error {
	...
	if raw, ok := fhir.TakeRawField(obj, "_additiveProductName"); ok {
		var element fhir.PrimitiveElement
		if err := json.Unmarshal(raw, &element); err != nil {
			return err
		}
		v.AdditiveProductNameElement = &element
	}
