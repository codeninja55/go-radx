...
type Coverage struct {
	...
	SubscriberIdElement *fhir.PrimitiveElement        `json:"-"`
...
func marshalCoverageSiblings(v *Coverage, encoded []byte) ([]byte, error) {
	...
	if raw, err := fhir.MarshalPrimitiveExtension(v.SubscriberIdElement); err != nil {
		return nil, err
	} else {
		siblings = append(siblings, fhir.RawSibling{Key: "_subscriberId", Value: raw})
	}
...
func (v *Coverage) UnmarshalJSON(data []byte) error {
	...
	if raw, ok := fhir.TakeRawField(obj, "_subscriberId"); ok {
		var element fhir.PrimitiveElement
		if err := json.Unmarshal(raw, &element); err != nil {
			return err
		}
		v.SubscriberIdElement = &element
	}
