...
// SubstanceInstance is a generated nested backbone element.
...
// their value arrays.
func (v SubstanceInstance) MarshalJSON() ([]byte, error) {
	type alias SubstanceInstance
	encoded, err := json.Marshal(alias(v))
	if err != nil {
		return nil, err
	}
	return marshalSubstanceInstanceSiblings(&v, encoded)
}

// marshalSubstanceInstanceSiblings appends the primitive "_field" siblings to the
// already-encoded object, preserving the value fields' canonical order: an empty
// scalar sibling and an un-extended repeating sibling are skipped, and a repeating
// sibling's array is null-aligned with its value array.
func marshalSubstanceInstanceSiblings(v *SubstanceInstance, encoded []byte) ([]byte, error) {
	var siblings []fhir.RawSibling
...
func (v *SubstanceInstance) UnmarshalJSON(data []byte) error {
	obj, err := fhir.SplitRawObject(data)
	if err != nil {
		return err
	}
