// This file is HAND-WRITTEN, not generated. It owns the R5 release's own
// fhir.Registry instance and the release-scoped entry points that read it. The
// generated registry.go and validation_descriptors.go register every R5 resource's
...
// own Registry resolves that: r5.UnmarshalResource always returns R5 types, and an R5
...
// Registry is the R5 release's resourceType-keyed factory, validation, and summary
...
// UnmarshalResource decodes FHIR JSON into the concrete R5 resource named by its
// "resourceType". The returned Resource holds an R5 dynamic type (for example
// *r5.Patient). A payload whose "resourceType" is absent, empty, or not an R5 resource
...
// UnmarshalResourceSlice decodes a JSON array of FHIR resource objects into R5
// resources, dispatching each element through UnmarshalResource. It backs the decode of
// a repeating resource-typed R5 field (DomainResource.contained). A JSON null yields a
// nil slice; any element whose resourceType is absent, empty, or not an R5 resource
...
// Unmarshal decodes FHIR JSON into the concrete R5 resource type T, verifying the
// payload's "resourceType" matches T before decoding. It is fhir.Unmarshal pinned to
// the R5 release for symmetry with the other release-scoped entry points; because
// fhir.Unmarshal resolves T by its static type rather than the registry, it does not
// depend on R5 registration.
func Unmarshal[T fhir.Resource](data []byte) (T, error) {
	return fhir.Unmarshal[T](data)
}

// Validate runs go-radx's structural and binding-level validation over an R5 resource
// and returns the issues as an *fhir.OperationOutcome, consuming the R5 validation
// descriptors registered in Registry. A resource whose type has no registered R5
...
// MarshalSummary serializes an R5 resource under the given summary mode, consuming the
// R5 summary descriptors registered in Registry. A resource whose type has no
// registered R5 summary descriptor is returned in full rather than filtered.
